    ...
    if(isset($_POST['post']))
    {
           ...
           $qry=mysqli_query($con,"SELECT * FROM booking WHERE date(book_for)='$bookfor' and `time`='$time'") or die(mysqli_error($con));
            ...
            if($count <=0)
            {
                ...
                $nm=mysqli_query($con,"SELECT * FROM shop_owners WHERE shop_name='$sname'");
                $num=mysqli_fetch_array($nm);
                $name=$num['owner'];

                  $sql=mysqli_query($con,"INSERT INTO `booking`( `hall_id`, `book_by`,`book_for`, `booked_on`, `time`, `book_comment`, `event`, `book_status`) VALUES ('$hid','$name','$bookfor',now(),'$time','$bookdesc','$event','booked')") or die(mysqli_error($con));
                            if($sql)
                            {
